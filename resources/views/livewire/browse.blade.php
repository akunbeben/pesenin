<div class="max-h-screen px-2 mx-auto sm:max-w-sm" x-data="{ showingModal: @entangle('showModal') }">
    <div class="grid grid-cols-2 gap-2.5 py-5">
        @foreach ($products as $product)
        <div class="bg-white border border-gray-200 shadow-xs cursor-pointer rounded-xl" x-on:click="$dispatch('show-product', { product: {{ $product->id }} })">
    ...
    <x-filament::modal slide-over id="product-detail">
        <div class="relative">
            <button type="button" class="fixed rounded top-5 left-5 bg-gray-100/50 md:hidden" x-on:click="$dispatch('close-modal', { id: 'product-detail' })">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                </svg>
            </button>

            <button type="button" class="fixed p-1 rounded-full top-5 right-5 bg-gray-100/50" x-on:click="$dispatch('close-modal', { id: 'product-detail' })">
        ...
        <div class="flex flex-col p-3 grow gap-y-5">
            ...
            <div class="flex w-full mt-auto bottom-5">
                <x-filament::button
                    ...
                    x-on:click="$dispatch('add-to-cart', { product: '{{ $showed?->id }}' })"
                >
                    Add to cart
                </x-filament::button>
            </div>
